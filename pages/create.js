...
export default function Create() {
    ...
    const [mining, setMining] = useState(false);//loading states
    ...
    const submitCreate = async (e) => {
        ...
        setMining(true);
        try {//if transaction/connection somehow fails, don't get stuck in loading state

            const createTxn = await keyboardsContract.create(keyboardSize, isPBT, filter)
            ...
            <div>
                <div className="flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 p-2">
                        <Keyboard size={keyboardSize} isPBT={isPBT} filter={filter} />
                    </div>
                </div>
            </div>

            <PrimaryButton type="submit" onClick={submitCreate} disabled={mining}>
                {mining ? <span>Processing&nbsp;&nbsp;<span className="animate-ping relative inline-flex rounded-full h-3 w-3 bg-purple-500"></span></span> :  "Create"}
            </PrimaryButton>
            </form>
